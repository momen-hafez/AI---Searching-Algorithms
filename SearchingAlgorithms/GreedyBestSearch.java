...
public class GreedyBestSearch {
	...
	public GreedyBestSearch(Map<Pair<String, String>, InputParameters> params, Map<String, ArrayList<String>> adj, int numOfGoals,String[] goals, String start, String[][] vals) {
		super();
		this.params = params;
		this.adj = adj;
	...
	public static void setFinalPath(ArrayList<String> finalPath) {
		GreedyBestSearch.finalPath = finalPath;
	}
	...
	public static void setVisited(Map<String, Boolean> visited) {
		GreedyBestSearch.visited = visited;
	}
	...
	public static void setPath(ArrayList<Pair<Pair<String, String>, Double>> path) {
		GreedyBestSearch.path = path;
	}
	public static boolean isFound() {
		return isFound;
	}
	public static void setFound(boolean isFound) {
		GreedyBestSearch.isFound = isFound;
	}
	...
	double prev = 0;
	public static void GBS (String start, Map<String,ArrayList<String> > adj , int numOfGoals, String[]goals , Map<Pair<String,String>,InputParameters> params , Map<String,ArrayList<Double> > h, 
			double prev , int idx) {
		if(start.equals(goals[idx])) {
			...
			for(int i = finalPath.size() - 1 ; i >= 0 ; i--) {
				System.out.println(" GBS --> " + finalPath.get(i));
			}
			...
			if(visited.get(start) == false && isFound == false) {
				...
				for(int i = 0 ; i < adj.get(start).size() ; i++) {
					if(visited.get(adj.get(start).get(i)) == false) {
						...
						f.add(new Pair<Pair<String,String>,Double>(new Pair<String,String>(adj.get(start).get(i),start),H + prev));
					}
				}
			 }
			double mn = 1e7;
			...
			if (nextCity != null) {
			    ...
			    if(isFound == false)
			    	GBS(nextCity, adj, numOfGoals , goals, params , h , prev + params.get(new Pair<String,String>(nextCity,start)).getStreetDistance() , idx);
			}
